...
public class ActionHpBarController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (actionStats == null)
        {
            Debug.LogError($"[ActionHpBarController] ActionStats component not found on {gameObject.name}");
            enabled = false;
        }
    }
    ...
    private void Start()
    {
        if (!showHpBar || actionStats == null) return;
        InitializeHpBar();
        SubscribeToEvents();
    }
    ...
    private void InitializeHpBar()
    {
        ...
        // Điều này đảm bảo rằng HP bar được khởi tạo sau khi ActionStats đã sẵn sàng
        if (actionStats != null)
        {
            currentHealthPercentage = actionStats.GetHealthPercentage();
    ...
    {
        actionStats.OnDestroyed += OnActionDestroyed;
        actionStats.OnHpChange += OnHpChanged;
    }
    ...
    private void OnActionDestroyed(GameObject action)
    {
    ...
    private void UpdateHpBarVisibility()
    {
        ...
        if (hideWhenDead && actionStats.IsDestroyed) shouldBeVisible = false;
    ...
    {
        if (!isInitialized || actionStats == null) return;

        targetHealthPercentage = actionStats.GetHealthPercentage();
