...
public class GameProgressManager : MonoBehaviour
{
	...
	public void LoadOrInitialize()
	{
		if (File.Exists(SavePath))
		{
			...
			CurrentWorld = JsonUtility.FromJson<WorldData>(json);
		}
		else
		{
	...
	{
		foreach (var floor in CurrentWorld.floors)
		{
			floor.isUnlocked = false;
		...
		var firstFloor = CurrentWorld.floors.OrderBy(f => f.floorId).FirstOrDefault();
		if (firstFloor != null)
		{
			firstFloor.isUnlocked = true;
			var firstLevel = firstFloor.levels.OrderBy(l => l.levelId).FirstOrDefault();
			if (firstLevel != null)
			{
				firstLevel.isUnlocked = true;
			}
